...
4. **オーナーレビュー**: タスク計画が完成したら、**実行を開始する前に**オーナーにレビューを依頼する
   - タスクファイルの内容を提示し、承認を求める
   - オーナーの承認が得られるまで実行に進まない
   - フィードバックがあればtask-managerに修正を委譲し、再度レビューを依頼する
...
1. **オーナーレビュー**: 設計書の内容をオーナーに提示し、レビューと承認を依頼する
   - 設計書の全体像、主要な設計判断、トレードオフを報告する
   - **オーナーの承認が得られるまで次のステップに進まない**
   - フィードバックがあればsystem-designerに修正を委譲し、再度レビューを依頼する
2. **タスク計画への影響を評価**: 承認された設計書の「実装への影響」セクションを確認
3. **必要に応じてtask-managerに再計画を委譲**: 設計結果に基づいてタスク計画の更新が必要な場合
   - 承認済み設計書の内容をコンテキストとしてtask-managerに提供
   - task-managerが設計に基づいた具体的な実装タスクを作成/更新
   - 更新されたタスク計画も再度オーナーにレビューを依頼する（「2. タスク計画」のステップ4参照）
