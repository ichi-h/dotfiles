...
調査タイプ・タスク受取形式・スコープ厳守ルールは **investigationスキル** を参照。
...
> **タイプ別委譲で受けた場合**: 委譲されたタイプのスコープ内のみで以下のステップを実行すること（investigationスキルのスコープ厳守ルール参照）。スコープ外の調査が必要な場合は、現時点での発見を報告しつつ追加調査タイプを提案する。
