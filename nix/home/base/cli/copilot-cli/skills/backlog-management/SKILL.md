...
- 手動テスト・検証セクションは明示的に要求されない限り含めない
...
| 基準 | 良い例 | 悪い例 |
|---|---|---|
| 動詞始まり | 「`auth/token.ts` のJWT検証ロジックを修正する」 | 「JWT関連の問題」 |
| ファイルリンク | 変更対象ファイルを明示する | ファイル名なし |
| シンボル参照 | 関数名・クラス名・変数名を含める | 「適切な実装をする」 |
| スコープ明確 | 単一の責務に絞る | 「全体的に改善する」 |
